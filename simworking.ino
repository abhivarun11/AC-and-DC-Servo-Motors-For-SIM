...
// ESP32-S3 pins: 2, 4, 5, 6, 7, 8, 9, 10, 13, 14, 15, 16, 17, 18, 21, 38, 39, 40 are all safe and have pull-ups
const AxisConfig config[AXES_TOTAL] = {
  { 2,  4, 13 }, // Axis 0 (FlyPT Axis 1)
  { 7,  14, 5 }, // Axis 1 (FlyPT Axis 2)
  { 8,  9, 10 }, // Axis 2 (FlyPT Axis 3)
  {40, 41, 42 }, // Axis 3 (FlyPT Axis 4)
...
static const int32_t HOMING_BACKOFF_REVS = 6;
...
// =====================================================
static const uint8_t ESTOP_PIN = 47; // DO NOT USE PIN 0 for an NC button (it prevents booting). Pin 47 is a safe, free pin.
static const int ESTOP_ACTIVE_STATE = HIGH; // HIGH means E-Stop triggered (Normally Closed button)
...
// =====================================================
// AUTO-DETECT PARSER STATE (FLYPT & SIMHUB)
// =====================================================
enum ParseMode { MODE_UNKNOWN, MODE_FLYPT, MODE_SIMHUB };
ParseMode currentParseMode = MODE_UNKNOWN;

// FlyPT State
enum ParseState { FIND_SYNC1, FIND_SYNC2, READ_PAYLOAD };
ParseState flyptState = FIND_SYNC1;
...
// =====================================================
// PARSE SERIAL (AUTO-DETECT FORMAT)
// =====================================================
void parseStream() {
  while (Serial.available() > 0) {
    uint8_t b = (uint8_t)Serial.read();

    if (currentParseMode == MODE_UNKNOWN) {
      if (b == SYNC1) {
        currentParseMode = MODE_FLYPT;
        flyptState = FIND_SYNC2;
        continue;
      } else if ((b >= '0' && b <= '9') || b == '-') {
        currentParseMode = MODE_SIMHUB;
        bufferIndex = 0;
        serialBuffer[bufferIndex++] = (char)b;
        continue;
      }
    }

    if (currentParseMode == MODE_FLYPT) {
      switch (flyptState) {
        case FIND_SYNC1:
          if (b == SYNC1) flyptState = FIND_SYNC2;
          else if (b == '\n') currentParseMode = MODE_UNKNOWN;
          break;

        case FIND_SYNC2:
          if (b == SYNC2) {
            flyptState = READ_PAYLOAD;
            payloadIndex = 0;
          } else if (b == SYNC1) {
            flyptState = FIND_SYNC2;
          } else {
            flyptState = FIND_SYNC1;
            currentParseMode = MODE_UNKNOWN;
          }
          break;

        case READ_PAYLOAD:
          if (b == SYNC1 && payloadIndex == 0) {
            flyptState = FIND_SYNC2;
            break;
          }
          ...
          if (payloadIndex >= PAYLOAD_BYTES) {
            ...
            flyptState = FIND_SYNC1;
            payloadIndex = 0;
            currentParseMode = MODE_UNKNOWN; // Reset to allow switching
          }
          break;
      }
    } else if (currentParseMode == MODE_SIMHUB) {
      char c = (char)b;
      if (c == '\n') {
        ...
        while ((token = strtok_r(ptr, ",", &ptr)) != NULL && axisIdx < AXES_TOTAL) {
          uint16_t rawVal = (uint16_t)atoi(token);
          if (axes[axisIdx].runMode == MODE_ACTIVE) {
            int32_t mapped = mapU16ToPulsesNeutralCenter(rawVal);
            commandTarget(axisIdx, mapped);
          }
          axisIdx++;
        }
...
void printStatus() {
  ...
  for (int i=0; i<AXES_TOTAL; i++) {
    ...
    else if (axes[i].runMode == MODE_ESTOP) Serial.print("EST");
...
void loop() {
  static uint32_t lastEstopChangeMs = 0;
  static int lastEstopState = digitalRead(ESTOP_PIN);
  int currentEstopState = digitalRead(ESTOP_PIN);

  if (currentEstopState != lastEstopState) {
    lastEstopChangeMs = millis();
    lastEstopState = currentEstopState;
  }

  if (millis() - lastEstopChangeMs > 50) { // 50ms debounce
    if (!estopTriggered && currentEstopState == ESTOP_ACTIVE_STATE) {
      estopTriggered = true;
    ...
    } else if (estopTriggered && currentEstopState != ESTOP_ACTIVE_STATE) {
      // E-Stop released, start homing again
      estopTriggered = false;
      Serial.println("EMERGENCY STOP RELEASED. RESTARTING HOMING...");
      for (int i = 0; i < AXES_TOTAL; i++) {
        axes[i].runMode = MODE_HOMING_SEEK;
        axes[i].switchStableSeen = false;
        axes[i].switchSeenMs = 0;
